...
const HeaderIntroduction: React.FC = () => {
  ...
  return (
    <div>
      <h1 className="text-8xl font-bold pt-30 text-center text-white italic">KIẾN GROUP</h1>
      <h2 className="text-3xl font-bold py-10 text-center text-white italic">- Kiến tạo không gian Việt -</h2>
      <div className="text-xl bgBlue text-white rounded-2xl w-[80%] mx-auto px-10 py-2 my-10 shadow-2xl hover:shadow-blue-950 transition-shadow duration-300">
          ...
          {attribute.map((item, index) => {
            return (
              <Button
                ...
                key={index}
                className={clsx("px-2 py-2 rounded-sm cursor-pointer hover:opacity-50 hover:shadow transition-all duration-300 bgBrown border-4",attr?.name === item.name ? 'border-b-red-200':'border-transparent')}
              >
                {item.name}
              </Button>
            );
        ...
        <div className="bgBrown p-5 rounded-md my-20 border-4 ">
          {attr ? (
            <>
              <p className="text-2xl font-bold text-center my-5 border-b-2 py-2">{attr.slogan}</p>
              {attr.content.map((e, index) => {
                return <p key={index} className='px-10'>{e}</p>;
              })}
            </>
          ) : (
            <p className="text-2xl font-bold text-center my-5">Chọn điều bạn muốn tìm hiểu</p>
          )}
        </div>
        ...
        <div className='w-[80%] mx-auto overflow-hidden object-cover rounded-xl relative my-10'>
