...
const HomeDiffer: React.FC = () => {
  return (
    <div className="py-10 w-full ">
      <h1 className="text-5xl font-bold my-20 w-fit mx-auto text-white">Sự khác biệt</h1>
      <div className="bgBlue w-[80%] mx-auto px-10 py-10 rounded-4xl">
        <h1 className="text-3xl font-bold my-10 text-white py-2">
          Sự chuẩn hóa từ thiết kế, đến các bước triển khai kỹ thuật thi công
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          {difference.map((dif,index) => {
            return (
              <div key={index} className="group bg-white p-10 rounded-4xl text-blue-950 shadow-lg shadow-blue-700 border-8 border-transparent hover:border-blue-500 transition-colors duration-300">
                <div className="text-7xl group-hover:animate-bounce ">{dif.icon}</div>
                <h2 className="text-2xl font-bold my-5">{dif.name}</h2>
                <p className="text-lg">{dif.content}</p>
              </div>
            );
          })}
        </div>
        <h1 className="text-3xl font-bold my-20 text-white">Quy trình thiết kế</h1>
        <div className="flex  flex-col md:flex-row">
          {steps.map((step, index) => {
            return (
              <div key={index} className="group p-5 text-white border-l-2 md:border-t-2 md:border-l-0  w-full relative">
                <div className="absolute text-3xl top-0 left-0 w-10 h-1 md:h-10 md:w-1 bg-white"></div>
                <h1 className="text-5xl font-bold my-5">{step.name}</h1>
